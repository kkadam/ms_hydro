...
!*
!*****************************************************************************
subroutine swap_r_z(xr, xz)
implicit none
include 'runhydro.h'
include 'mpif.h'
!*****************************************************************************
!*
!  swap_r_z takes an array that is distributed with:
...
!  copy iam's own section of xr to xz
do L = 1, numphi_dd
   do K = 1, numz_dd
      do J = 1, numr_dd_z
         xz(J,K+iam_mod*(numz_dd-2),L) = xr(J+iam_mod*(numr_dd_z-2),K,L)
      enddo
   enddo
enddo

!  now do the message passing to exchange the rest of xr and xz
!  have to communicate numz_procs - 1 times to fill in the rest of
!  xz on all processors.  
...
do I = 1, numz_procs - 1
   ...
   do L = 1, numphi_dd
      do K = 1, numz_dd
         do J = 1, numr_dd_z
            send_buffer(J,K,L) = xr(J+send_to_mod*(numr_dd_z-2),K,L)
         enddo
      enddo
   enddo

   call mpi_irecv(receive_buffer, message_length, REAL_SIZE, receive_from(index),  &
                  I*4000+iam, MPI_COMM_WORLD, irequest_recv, ierror)

   call mpi_isend(send_buffer, message_length, REAL_SIZE, send_to(index),          &
                  I*4000+send_to(index), MPI_COMM_WORLD, irequest_send, ierror)
   ...
   do L = 1, numphi_dd
      do K = 1, numz_dd
         do J = 1, numr_dd_z
            xz(J,K+receive_from_mod*(numz_dd-2),L) = receive_buffer(J,K,L)
         enddo
      enddo
   enddo
