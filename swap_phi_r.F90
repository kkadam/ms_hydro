...
do L = 1, numphi_dd
   do K = 1, numz_dd
      do J = 1, numr_dd
         xr(J+iam_mod*(numr_dd-2),K,L) = xphi(J,K,L+iam_mod*numphi_dd)
      enddo
   enddo
enddo
...
do I = 1, numr_procs - 1
   ...
   do L = 1, numphi_dd
      do K = 1, numz_dd
         do J = 1, numr_dd
            send_buffer(J,K,L) = xphi(J,K,L+send_to_mod*numphi_dd) 
         enddo
      enddo
   enddo

   call mpi_irecv(receive_buffer, message_length, REAL_SIZE, receive_from(index),  &
                  I*3000+iam, MPI_COMM_WORLD, irequest_recv, ierror)

   call mpi_isend(send_buffer, message_length, REAL_SIZE, send_to(index),          &
                  I*3000+send_to(index), MPI_COMM_WORLD, irequest_send, ierror)
   ...
   do L = 1, numphi_dd
      do K = 1, numz_dd
         do J = 1, numr_dd
            xr(J+receive_from_mod*(numr_dd-2),K,L) = receive_buffer(J,K,L)
         enddo
      enddo
   enddo
